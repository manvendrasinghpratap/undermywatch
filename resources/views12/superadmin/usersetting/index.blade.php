		 ...
		 <div class="content-wrapper">
				<div class="content-heading">
					 ...
					 <div class="ml-auto" style="margin-top: -21px;">
								...
								<a href="{{route('superadmin.usersetting.campaign',['userId' => base64_encode(@$userDetails['id'])])}}" class="btn btn-default btn-lg dash-navi"><em class="fa fa-mask success-circle"></em> Campaigns</a>
								<a href="{{ route('admin.stats.userstatsByUserId', ['userId' => base64_encode(@$userDetails['id'])]) }}?tab=shield" class="btn btn-default btn-lg dash-navi"><em class="fa fa-chart-line success-circle"></em> Reports</a> 
								<a href="{{ route('superadmin.usersetting.user', ['user'=>base64_encode(@$userDetails['id'])]) }}?tab=profile" class="btn btn-default btn-lg dash-navi"><em class="fa fa-user-ninja success-circle"></em> Profile</a>
					 </div>
				</div>
								 ...
								 <div class="tab-content p-0 b0">
										<div class="tab-pane @if($active=='profile') active @endif" id="tabSetting1">
											 <div class="card b">
													...
													<div class="card-body">
															 <form  action="{{ route('superadmin.usersetting.updateprofile', ['userId' => base64_encode(@$userDetails['id'])]) }}" method="POST" enctype="multipart/form-data">
				                        {{ csrf_field() }}
															 ...
															 @foreach($userDetails->sections as $section)
																...
																<div class="list-group-item d-flex align-items-center">
																	<?php	if($section['image']==''){
																	 ...
																	 <div class="ml-auto">
																		 <form method="post" action="{{route('superadmin.usersetting.deleteUserSection',[base64_encode($section['id']),'&?tab=shield'])}}">
                                    ...
                                    <div class="list-group-item d-flex align-items-center">
                                       ...
                                       <div class="ml-auto">
																				 <form method="post" action="{{route('superadmin.usersetting.assignUserSection',[base64_encode($section['id']),'&?tab=shield'])}}">
																					 <input type="hidden" name="user_id" value="{{base64_encode($userDetails['id'])}}"/>
																					 <input type="hidden" name="section_id" value="{{base64_encode($section['id'])}}"/>
