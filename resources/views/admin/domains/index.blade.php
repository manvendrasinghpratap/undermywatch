	 ...
	 <div class="content-wrapper">
			...
			<div class="row">
				@include('admin.components.statuses')
				 ...
				 <div class="card-body">
					 <form method="POST" action="{{ route('admin.domains.create') }}">
							 {{ csrf_field() }}
							 <div class="form-row align-items-center">
									...
									<div class="col-lg-3">
										<select  class="form-control is_public" name="is_public" id="select2-1">
 									 <option value="0"  >Public</option>
									 <option value="1" selected = true>Private</option>
 									 <option value="2" selected = true>Wordpress</option>
 									 </select>
									</div>
													 ...
													 <tr>
															 ...
															 <td>
																		 ...
																		 @if($domain->enable_log)
																				 <form action="{{ route('admin.domains.disablelog', ['domain' => $domain->id]) }}" method="POST" style="display:inline-block;">
																						 {{ csrf_field() }}
																						 <button type="submit" class="btn btn-xs btn-primary"><i class="fa fa-link"></i></button>
																				 </form>
																		 @else
																		 <form action="{{ route('admin.domains.enablelog', ['domain' => $domain->id]) }}" method="POST" style="display:inline-block;">
																			 {{ csrf_field() }}
																			 <button type="submit" class="btn btn-xs btn-success"><i class="fa fa-link"></i></button>
																		 </form>
																		 @endif
																		 <a href="{{ route('admin.logs.domain', ['domain'=> $domain->id]) }}" class="btn btn-secondary btn-xs"><i class="fa fa-times-circle"></i></a>
																		 <a href="{{ route('admin.logs.domain', ['domain'=> $domain->id]) }}" class="btn btn-secondary btn-xs"><i class="fa fa-eye"></i></a>
																		 @if(Auth::user()->id == @$domain->addedby->id || Auth::user()->level > 1)
																				 <form action="{{ route('admin.domains.delete', ['domain' => $domain->id]) }}" method="POST" style="display:inline-block;">
																						 {{ csrf_field() }}
																						 <button style="" type="submit" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></button>
																						 <button style="display:none;" type="button" class="btn btn-danger btn-xs confirmDelete" data-siteurl ="{{ url('/')}}" data-tablename="users" data-record-id="{{ $domain->id }}" data-record-title="Are you sure you want to delete this User ?" data-toggle="modal" data-target="modal-confirm" data-succuss="User deleted successfully" data-deletetype="softdelete">
			                                          <i class="fa fa-trash"></i></button>
																				 </form>
																		 @endif
															 </td>
															 <td>
																	 <form action="{{ route('admin.domains.update', ['domain' => $domain->id]) }}" method="POST">
																			 {{ csrf_field() }}
