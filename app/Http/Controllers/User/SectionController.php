...
class SectionController extends Controller
{
    ...
    public function index(Request $request){
      ...
      $links = Links::where('createdby_id',Auth::user()->id)->where('section_id',base64_decode($request->query('section_id')))->get();
      }
      else {
      $links = Links::where('createdby_id',Auth::user()->id)->get();
      }
      $sectionList =  UserSection::where('user_id', Auth::user()->id)->pluck('section_id','section_id');
      $sections = Sections::whereIn('id', $sectionList)->orderby('created_at','desc')->get();
      $sections = $sections->toArray();
      return view('user.sections.index')->with('user', $this->user())->with('links', $links)->with('sections',$sections)->with('querySectionString',$querySectionString);

      //  return view('user.sections.index')->with('user', $this->user());
    }
    ...
    public function section(Sections $section, Request $request){
        ...
        return view('user.sections.section')->with('section', $section)->with('link', $links)->with('domains', $domains)->with('landingpages', $landingpages);
    }
    ...
    public function link(Links $link, Request $request){
      ...
      $domains = Domains::latest()->get();
      $landingpages = $this->dirs;
      return view('user.sections.link')->with('link', $link)->with('domains', $domains)->with('landingpages', $landingpages);
