    ...
    public function create(Request $request){
      ...
      $slug = $request->get("slug", "") ?: "";
    	...
    	if(empty($slug) || !empty($slug_exists)){
          ...
          // return back()->withInput($request->all)->withErrors('Add atleast one image in the post.');
          // return redirect()->back()->withInput()->withErrors('Add atleast one image in the post.');
          // $this->pp($request->all(),1);
          // die();
          return redirect()->back()->withInput($request->all)->with('error', 'Slug Already Exists');
    		/*	return back()
	    			->withInput()
        ...
        foreach ($superAdminArray as $superadminuserid) {
          ...
          $insertSuperAdminUserIdInUserSectionTbl->save();
    		...
    		if($request->ajax()){
    			...
    			return redirect()->route('superadmin.sections.index')->with('status','Section Created Successfully');
    		}
    	}
    	if($request->ajax()){
